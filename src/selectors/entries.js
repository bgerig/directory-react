...
export const getVisibleEntries = (entries, filters, entryType, team) => {
	...
	visibleEntries = entries.filter((entry) => {
		...
		const matchesQuery =
			queryStringMatches(entry.firstName) || queryStringMatches(entry.lastName) ||
			queryStringMatchesFullName(entry.firstName, entry.lastName) || queryStringMatches(entry.roomName) ||
			queryStringMatches(entry.otherName) || queryStringMatches(entry.phoneExtension) || queryStringMatches(entry.email) ||
			queryStringMatches(entry.position) || queryStringMatchesTeam(entry.team) || queryStringMatches(entry.discipline) ||
			queryStringMatches(entry.college) || queryStringMatches(entry.hometown);
