import { addEntry, editEntry, removeEntry, startAddEntry, startSetEntries, setEntries, startRemoveEntry, startEditEntry } from '../../actions/entries'
...
test('should fetch entries from firebase', (done) => { //we pass done() since this is an asynchronous test case
	const store = createMockStore({});
	store.dispatch(startSetEntries()).then(() => { //we can use then() here because the call to the database is returning a promise chain
...
test('should add entry to database and store', (done) => {
	...
	store.dispatch(startAddEntry(entryData)).then(() => { //we can use then() here because the call to the database is returning a promise chain
		const actions = store.getActions();
		expect(actions[0]).toEqual({
			type: 'ADD_ENTRY',
			entry: {
...
test('should add entry with defaults to database and store', (done) => {
	const store = createMockStore({});
	const entryData = {}
	store.dispatch(startAddEntry(entryData)).then(() => { //we can use then() here because the call to firebase is returning a promise chain
		const actions = store.getActions();
		expect(actions[0]).toEqual({
			type: 'ADD_ENTRY',
			entry: {
...
test('should remove entry from store and firebase', (done) => {
	...
	store.dispatch(startRemoveEntry({ id: id })).then(() => {
		const actions = store.getActions();
		expect(actions[0]).toEqual({
			type: 'REMOVE_ENTRY',
			id: id
...
test('should edit entry from store and firebase', (done) => {
	...
	store.dispatch(startEditEntry({ id, updates })).then(() => {
		const actions = store.getActions();
		expect(actions[0]).toEqual({
			type: 'EDIT_ENTRY',
			id: id,
